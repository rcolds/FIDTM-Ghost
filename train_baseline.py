...
import time
from Networks.Ghost_Net.ghostnet import ghostnet
...
def main(args):
    ...
    criterion = (nn.MSELoss(size_average=False).cuda(),pytorch_ssim.SSIM(window_size=11))
...
def train(Pre_data, model, criterion, optimizer, epoch, args):
    ...
    with open('losses.txt', 'a') as f:
        f.write('Epoch{},loss:{}\n'.format(epoch,losses.avg))  # 文件的写操作
...
def generate_bounding_boxes(kpoint, fname):
    ...
    Img_data = cv2.imread(
        'D:/deeplearning/dataset/shanghaitech/part_A_final/test_data/images/' + fname[0])
